...
for canteen in canteen_data:
    if canteen["canteen_name"] not in existing_store_names:
        existing_store_names[canteen["canteen_name"]] = [store["name"] for store in canteen["stores"]]

# --- Initialize Session State ---
if "selected_canteen" not in st.session_state:
    st.session_state.selected_canteen = existing_canteen_names[0] if existing_canteen_names else None
...
if st.session_state.editing_food_index is not None:
    canteen_index_edit = next((i for i, c in enumerate(canteen_data) if c["canteen_name"] == st.session_state.selected_canteen), None)
    if canteen_index_edit is not None:
        store_index_edit = next((i for i, s in enumerate(canteen_data[canteen_index_edit]["stores"]) if s["name"] == st.session_state.selected_store), None)
        if store_index_edit is not None:
            if st.session_state.editing_food_index < len(canteen_data[canteen_index_edit]["stores"][store_index_edit]["food_items"]):
                food_item = canteen_data[canteen_index_edit]["stores"][store_index_edit]["food_items"][st.session_state.editing_food_index]
...
canteen_selection = st.selectbox(
    ...
    index=existing_canteen_names.index(st.session_state.selected_canteen) if st.session_state.selected_canteen in existing_canteen_names else 0,
...
if st.session_state.selected_canteen:
    store_selection = st.selectbox(
        ...
        index=existing_store_names.get(st.session_state.selected_canteen, []).index(st.session_state.selected_store) if st.session_state.selected_store in existing_store_names.get(st.session_state.selected_canteen, []) else 0,
        key="store_selection"
    )
    ...
    if not st.session_state.selected_store:
        ...
        store_index = next((i for i, s in enumerate(canteen_data[canteen_index]["stores"]) if s["name"] == st.session_state.selected_store), None)

        # --- Input Fields for Food Items ---
        st.subheader("รายการอาหาร")
        food_name = st.text_input("ชื่ออาหาร", value=st.session_state.food_name, key=food_name_key)
        normal_price = st.number_input(
            "ราคา (ธรรมดา)", min_value=0.0, format="%.2f", value=st.session_state.normal_price, key=normal_price_key
        )
        special_price = st.number_input(
            "ราคา (พิเศษ)", min_value=0.0, format="%.2f", value=st.session_state.special_price, key=special_price_key
        )
        ...
        button_label = "บันทึกการแก้ไข" if st.session_state.editing_food_index is not None else "เพิ่มรายการอาหาร"
        if st.button(button_label):
            if st.session_state.food_name and st.session_state.normal_price is not None:
                ...
                if st.session_state.editing_food_index is not None:
                    canteen_data[canteen_index]["stores"][store_index]["food_items"][st.session_state.editing_food_index] = food_item
                    st.success(f"แก้ไขรายการอาหาร {st.session_state.food_name} ในร้าน {st.session_state.selected_store} เรียบร้อยแล้ว!")
                    st.session_state.editing_food_index = None
                    st.session_state.food_name = ""
                    st.session_state.normal_price = 0.0
                    st.session_state.special_price = None
                else:
                    if "food_items" not in canteen_data[canteen_index]["stores"][store_index]:
                        canteen_data[canteen_index]["stores"][store_index]["food_items"] = []
                    canteen_data[canteen_index]["stores"][store_index]["food_items"].append(food_item)
                    st.success(f"เพิ่มรายการอาหาร {st.session_state.food_name} ในร้าน {st.session_state.selected_store} เรียบร้อยแล้ว!")
                    st.session_state.food_name = ""
                    st.session_state.normal_price = 0.0
                    st.session_state.special_price = None
        ...
        st.header(f"รายการอาหารของร้าน {st.session_state.selected_store} ({st.session_state.selected_canteen})")
        if "food_items" in canteen_data[canteen_index]["stores"][store_index]:
            for i, item in enumerate(canteen_data[canteen_index]["stores"][store_index]["food_items"]):
                ...
                with col3:
                    if st.button(f"ลบ", key=f"delete_food_{i}"):
                        canteen_data[canteen_index]["stores"][store_index]["food_items"].pop(i)
                        save_canteen_data(canteen_data)
                        st.session_state.editing_food_index = None
                        st.rerun()
else:
